...
export class ExtensionService {
    ...
    async upload(filePath: string, options?: {
        ...
        // Upload to TestMu AI cloud if credentials available
        ...
        if (shouldUpload && this.config.username && this.config.accessKey) {
            try {
                ...
                this.log(`Uploaded to TestMu AI cloud: ${cloudUrl}`);
            } catch (error: any) {
    ...
    async uploadToLambdaTest(filePath: string): Promise<string> {
        if (!this.config.username || !this.config.accessKey) {
            throw new Error('TestMu AI credentials not configured');
        }
        ...
        if (!response.ok) {
            throw new Error(`TestMu AI API error: ${response.status} - ${responseText}`);
        }
        ...
        if (result.data && Array.isArray(result.data) && result.data.length > 0) {
            ...
            if (entry.error) {
                throw new Error(`TestMu AI upload error: ${entry.error}`);
            }
            if (entry.s3_url) {
                return entry.s3_url;
            }
        ...
        throw new Error(`No S3 URL returned from TestMu AI API. Response: ${responseText}`);
    }
